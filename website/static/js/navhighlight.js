...
(function() {
  ...
  const findHeadings = () => headingsCache ? headingsCache : document.querySelectorAll('.toc-headings > li > a');
    ...
    timer = setTimeout(() => {
      ...
      for (let i = 0; i < headings.length; i++) {
        ...
        if (!found && i < headings.length - 1) {
          ...
          // The following tests whether top + scrollTop (the top of the header) is greater than scrollTop
          // (where scrollTop = window.pageYOffset, the top of the window), with OFFSET pixels of slop.
          current = top > OFFSET;
        }
        if (current) {
          found = true;
