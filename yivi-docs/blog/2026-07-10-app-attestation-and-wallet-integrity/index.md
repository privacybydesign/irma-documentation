...
In April 2026 the European Commission published the source code for its **Age Verification App**, a white label reference wallet meant to show member states how to build privacy preserving age checks for the Digital Services Act. It was positioned as a blueprint for the wider EU Digital Identity Wallet program.

Within days it went badly wrong. A security researcher reset the app PIN and disabled biometric login in **under two minutes**, simply by editing a plain text preferences file on an Android device: delete the encrypted PIN entries, flip a boolean to turn off biometrics, and the stored credentials were still there for the taking. Another researcher then reproduced the work and documented several more issues, including personal data stored without encryption.
...
They are independent. A genuine app can hold a soft key, and a cloned app can talk to real hardware, so you need evidence for both. The ARF names three attestations that map onto those questions plus one more for presentation.
...
**The gap the ARF cares about: phones without Google or Apple.** Apple App Attest needs Apple, and Play Integrity needs Google services. A wallet running on GrapheneOS, CalyxOS or /e/OS, or installed from an independent Android distribution, has neither. For a sovereign European identity wallet those are not edge cases, they are requirements. So you need an **open** path, and there are two usable ones:

1. **Reuse the app identity that the key attestation already carries.** The Android key attestation extension already names the calling package and its signing certificate, rooted in a hardware root rather than in a store. Combined with the verified boot state it is a strong signal, and it works on any device with hardware attestation.
2. **A provider operated PKI.** The wallet provider runs a small certificate authority and issues each official build an attestation key and certificate. At activation the app signs the server nonce with that key, the server verifies the chain anchors in the provider root, and reads the app identity from the certificate.

The lesson is that app attestation is a pluggable idea. Apple and Google are two implementations of it, not the definition of it.
