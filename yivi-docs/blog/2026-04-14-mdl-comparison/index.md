...
In Austria, verification is registry-centric. There is a specific QR code for roadside checks, called “Verkehrskontrolle” (English: traffic stop). This QR code is scanned with an official police device, which triggers a check in the central driving license register. 

Australia takes a mixed approach. Verification is partly done via visual inspection, which is not much more than inspecting the mDL shown on the phone. Verification can also take place via a short-lived QR code, through which the validity of the license token is checked by authorized verifiers. However, documentation is limited, so it is not entirely clear whether this happens through a database. This suggests a more credential-centric approach, where the mDL is the primary source of trust. 
...
No active internet connection is needed on the holder’s side for a police check in either Austria or Australia. When the license is stored locally on the device, it can be shown offline. However, it is not entirely clear whether the police-side verification needs an internet connection. 
...
Australia also has context-dependent privacy protection, but with less transparency. Data minimization and selective disclosure are used in age verification, but in roadside checks, the full license is shown, just as handing over a physical license. It is, however, unclear how verification events are logged.

Germany takes a different approach. mDLs are not yet operational, largely due to privacy and security concerns. An earlier attempt to introduce mDLs was halted, which illustrates a cautious, law-first approach in which potential risks to privacy were considered unacceptable. Here, privacy acts as a blocking constraint instead of a design feature. 
