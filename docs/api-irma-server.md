...
Of these the latter three are *ending statuses*; once the session reaches such a status, it will not switch status again. A session starts as `"INITIALIZED"`. In the happy flow the status then becomes `"CONNECTED"`, and `"DONE"` after that when the session has successfully been completed.

---

### `GET /session/{token}/statusevents`

Subscribe to a [server-sent event](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events) stream of status updates. Whenever the session status changes, an event is sent with the new session status as a JSON string.
...
* `token`: Session token
* `status`: Current [session status](https://godoc.org/github.com/privacybydesign/irmago/server#Status)
* `type`: Session type: one of `"disclosing"`, `"signing"`, or `"issuing"`
...
If the session is not yet finished (that is, the session state is `INITIALIZED` or `CONNECTED` state), then only the first three fields are populated. (For getting just the current session status, use [`GET /session/{token}/statusevents`](http://localhost:3000/docs/api-irma-server#get-session-token-statusevents) or [`GET /session/{token}/status`](http://localhost:3000/docs/api-irma-server#get-session-token-status).)
...
If a JWT private key was [provided in the configuration of the `irma server`](irma-server#signed-jwt-session-results), then this returns a JWT signed by the `irma server` with the message from [`GET /session/{token}/result`](#get-session-token-result) above as JWT body, along with the following standard JWT fields:
